...
class BrowserTypeBase {
    ...
    async launch(options = {}) {
        ...
        const browser = await progress_1.runAbortableTask(progress => this._innerLaunch(progress, options, loggers, undefined), loggers.browser, timeoutSettings_1.TimeoutSettings.timeout(options), `browserType.launch`);
        return browser;
    }
    async launchPersistentContext(userDataDir, options = {}) {
        ...
        const loggers = new logger_1.Loggers(options.logger);
        const browser = await progress_1.runAbortableTask(progress => this._innerLaunch(progress, options, loggers, persistent, userDataDir), loggers.browser, timeoutSettings_1.TimeoutSettings.timeout(options), 'browserType.launchPersistentContext');
