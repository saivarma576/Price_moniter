...
class ElectronApplication extends events_2.EventEmitter {
    constructor(logger, browser, nodeConnection) {
        ...
        this._browserContext.on(events_1.Events.BrowserContext.Close, () => this.emit(exports.ElectronEvents.ElectronApplication.Close));
        ...
        return progress_1.runAbortableTask(async (progress) => {
            ...
            const { launchedProcess, gracefullyClose, kill } = await processLauncher_1.launchProcess({
                ...
                onExit: (exitCode, signal) => {
                    if (app)
                        app.emit(exports.ElectronEvents.ElectronApplication.Close, exitCode, signal);
                },
            });
            ...
            const browser = await crBrowser_1.CRBrowser.connect(chromeTransport, { headful: true, loggers, persistent: { viewport: null }, ownedServer: browserServer });
