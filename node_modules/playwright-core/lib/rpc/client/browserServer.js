...
class BrowserServer extends channelOwner_1.ChannelOwner {
    static from(server) {
        return server._object;
    }
    constructor(scope, guid, initializer) {
        super(scope, guid, initializer);
        this._channel.on('close', () => this.emit(events_1.Events.BrowserServer.Close));
    }
    process() {
        return { pid: this._initializer.pid };
    }
    wsEndpoint() {
